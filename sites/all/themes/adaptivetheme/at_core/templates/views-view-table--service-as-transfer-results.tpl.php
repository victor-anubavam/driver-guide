...
$searchFieldsArry= $searchFields->getSearchResult();
$i = 0;
...
$driv_name='';
$driv_num='';
if(isset($searchFields->_from) && trim($searchFields->_from)!='' && isset($searchFields->_to) && trim($searchFields->_to)!='')
{
    $from =  str_replace("'","\'",$searchFields->_from);
    $to =  str_replace("'","\'",$searchFields->_to);
    $result=db_query("SELECT field_add_region_family as driv_name, field_add_region_generational as driv_num 
        FROM `field_data_field_add_region` where field_add_region_given='".trim($from)."' and field_add_region_middle='".trim($to)."' limit 1 ");
    ...
    foreach ($result as $record) {   
     ...
     $night_rate_comm=db_query("SELECT fp.field_night_rate_percentage_value FROM 
         ...
         where fc.field_name ='field_percentage' AND  fr.field_night_rate_region_value='".trim($searchFields->_region)."' limit 1 ")->fetchField();
...
?>
 <table <?php if ($classes) { print 'class="'. $classes . '" '; } ?><?php print $attributes; ?>>
    ...
    <thead>
      <tr>
       <td class="history-lt" colspan="3">
           Region : <?php echo $searchFields->_region;?>&nbsp;&nbsp; - From : <?php echo $searchFields->_from; ?>&nbsp;&nbsp; - To : <?php echo $searchFields->_to; ?>
       </td>
       <td class="history-rt">&nbsp;
       </td>
      </tr>
  ...
  <tbody>
      ...
      <input type="hidden" name="from" id="from" value="<?php echo $searchFields->_from; ?>" />
      <input type="hidden" name="to" id="to" value="<?php echo $searchFields->_to; ?>" />
      <input type="hidden" name="region" id="region" value="<?php echo $searchFields->_region; ?>" />
...
if ((in_array(($row['nid']),$searchFieldsArry)) && (empty($searchFieldsArry['NR'])) && ($row['delta'] == $searchFieldsArry['delta'][$row['nid']])){    
        ...
        $filterArry[$record]['price'] = $searchFieldsArry['price'][$row['nid']];
...
foreach ($filterArry as $row) {    
 if ((in_array(($row['nid']),$searchFieldsArry)) && (empty($searchFieldsArry['NR'])) && ($row['delta'] == $searchFieldsArry['delta'][$row['nid']])){    
        ...
        $cartprice= $price;
        if($searchFields->_typeTransfer=='in&out')
        {
        ...
        <tr>       
          ...
          <td class="check-out">
           ...
           <?php echo $price."&nbsp;<b>&euro;</b>"; ?>
            ...
            {    
                if(isset($cart[$row['nid']]['product_details'][$searchFields->_typeTransfer])){?>
                <a href="javascript:void(0);" id="cart_<?php echo $row['nid'];?>" onclick="removefromCart('<?php echo $row['nid'];?>','<?php echo $cartprice; ?>','<?php echo $searchFields->_typeTransfer;?>')"><div class="remove">REMOVE FROM CART</div></a>
                <?php }else{ ?>
               <a href="javascript:void(0);" id="cart_<?php echo $row['nid'];?>" onclick="addtoCart('<?php echo $row['nid'];?>','<?php echo $cartprice; ?>','<?php echo $searchFields->_typeTransfer;?>')"> <div class="add">ADD TO CART</div></a>
                <?php }?>
            ...
            else
            {
              ?>
               ...
               <?php
              // SELECT fr.field_night_rate_region_value, fp.field_night_rate_percentage_value FROM field_collection_item as fc LEFT JOIN field_data_field_night_rate_region as fr ON fc.item_id=fr.entity_id LEFT JOIN  field_data_field_night_rate_percentage as fp ON fc.item_id=fp.entity_id  where fc.field_name ='field_percentage' AND  fr.field_night_rate_region_value='Paris'
            }
            ?>
