...
{  
    ...
    {  
        ...
        $gidmem = db_query("SELECT gid FROM `og_users_roles` WHERE uid ='".$memberArry->uid."' LIMIT 0 , 1")->fetchField();
        $query_mem = "SELECT DISTINCT og.etid FROM `og_membership` as og join node as n on og.etid=n.nid 
                        join log_detailed_transfer_results_checkout as logck on n.uid=logck.trav_agent_id  
                        WHERE og.gid ='".$gidmem."' and og.entity_type='node' $cond ";
        ...
        if($result_mem)
        {
            ...
            foreach ($result_mem as $row) {	
                $content_mem='<tr><th>Agency Name</th>
                ...
                $gross_margin_tot+=$gross_margin;
                 ...
                 $content_mem .='<tr><td class="tdtitle">'.$memberArry->title.'</td>
                    <td>'.$memberArry->field_hqfirst_name['und'][0]['value'].' '.$memberArry->field_last_name['und'][0]['value'].'</td>
                    <td class="location">'.$memberArry->field_hq_state['und'][0]['value'].'/'.$memberArry->field_street_adress['und'][0]['value'].'<br/>'.$memberArry->field_hq_city['und'][0]['value'].'-'.$memberArry->field_hq_zip['und'][0]['value'].'</td>
                    <td class="price">$ '.$globalbuss.'</td>
                    <td class="price">$ '.$gross_margin.'</td>
            ...
            if($content_mem!='')
            {
                $content_mem .='<tr class="lastTr"><td>Total</td>
                    <td ></td>
                    <td></td>
                    <td class="price">$ '.$global_buss_tot.'</td>
